...
namespace GameEngine
{
		...
		end_x(x), end_y(y), start_x(x), start_y(y)
	{
		...
		physics_type = PHYSICS_TYPE::STATIC; //move like kinematic
	}
	...
	void SelectionArea::Update()
	{
		...
		BoxShape* col = static_cast<BoxShape*>(collider);
		if (col) {
			if (!(rect.x == col->x && rect.y == col->y && rect.width == col->width && rect.height == col->height)) {
				col->width = rect.width;
				col->height = rect.height;
			}
		}
		else {
			collider = new BoxShape(0, 0, rect.width, rect.height);
		}
	}
